...
for (feature_num in feature_seq) {
  ...
  feature_unob <- feature_num / 2
  wt <- c(ATE, (1 : feature_unob) / cov_wt, intercep)
  logis_wt <- rep(c(1, -1), feature_unob / 2)
  for (sample_num in sample_seq) {
    filedir = paste0(path, feature_num, "/nonlinear", "/samplenum_", sample_num, "/")
    dir.create(filedir, showWarnings = F, recursive = T)
    for (k in 1 : filenum) {
      data_cov <- mvrnorm(sample_num, data_mu, data_sigma)
      odd_data <- data_cov[, seq(1, feature_num, 2)]
      even_data <- data_cov[, seq(2, feature_num, 2)]
      data_unob <- (odd_data + even_data + 1) ^ 2
      log_exp <- exp(data_unob %*% logis_wt + rnorm(sample_num))
      post_p <- 1 / (1 + log_exp)
      is_treat <- rbinom(post_p, 1, post_p)
      outcome <- cbind(is_treat, data_unob, 1) %*% wt + rnorm(sample_num, sd = noise_sigma)
